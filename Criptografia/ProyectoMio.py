...
def load_and_encrypt_any_file():
    filepath = filedialog.askopenfilename(title="Selecciona el archivo qeu deseea encriptar",filetypes=[("All files", "*.*"), ("Text files", "*.txt"), ("PDF files", "*.pdf"), ("Images", "*.png")])
    if filepath:
...
def decrypt_action():
    try:
        ...
        if filepath:
            ...
            key_filepath = filedialog.askopenfilename(title="Selecciona la calve secreta cifrada",filetypes=[("Key files", "*.bin;*.txt")])
            if not key_filepath:
