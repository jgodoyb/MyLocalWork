...
def load_and_encrypt_any_file():
    filepath = filedialog.askopenfilename(filetypes=[("All files", "*.*"), ("Text files", "*.txt"), ("PDF files", "*.pdf"), ("Images", "*.png")])
    if filepath:
...
def decrypt_action():
    try:
        filepath = filedialog.askopenfilename(filetypes=[("All files", "*.*"), ("Encrypted files", "*.enc"), ("Text files", "*.txt"), ("PDF files", "*.pdf"), ("Images", "*.png")])
        if filepath:
            with open(filepath, 'rb') as file:
                data = file.read()
            ...
            key_filepath = filedialog.askopenfilename(filetypes=[("Key files", "*.bin;*.txt")])
            if not key_filepath:
...
tk.Label(root, text="Tamaño de Clave (bits):").grid(row=0, column=0)
tk.OptionMenu(root, key_size_var, "128", "192", "256").grid(row=0, column=1)

tk.Label(root, text="Modo de Cifrado:").grid(row=1, column=0)
tk.OptionMenu(root, cipher_mode_var, "CBC", "CFB", "ECB").grid(row=1, column=1)

tk.Checkbutton(root, text="Usar RSA para cifrar clave AES", variable=rsa_option).grid(row=2, columnspan=2)

text_input = tk.Text(root, height=5, width=40)
text_input.grid(row=3, column=0, columnspan=2)

tk.Button(root, text="Cifrar Texto", command=encrypt_text).grid(row=4, column=0, sticky="ew")
tk.Button(root, text="Cargar y Cifrar Archivo", command=load_and_encrypt_any_file).grid(row=4, column=1, sticky="ew")
tk.Button(root, text="Descifrar Archivo", command=decrypt_action).grid(row=5, column=0, columnspan=2, sticky="ew")
